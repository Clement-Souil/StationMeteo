...
<section id="sql">
            ...
            <table border="1" align="center">
                <thead>
                    <tr><th colspan="7" style="align-self: center;"><p id="Dte">Actuellement</p></th></tr>
                </thead>
                <tbody>
                <tr>
                    <td>Ville</td>
		...
		$ville = $_GET['ville'];
                ...
                while ($row = mysqli_fetch_array($query)) {
                    ...
                    "<tr>
                    <td>{$row['info_lieu']}</td>
                    <td>{$row['info_temperature']}°C</td>
                    <td>{$row['info_meteo']}</td>
		    <td>{$row['info_pression']} Pa</td>
		    <td>{$row['info_humidite']}%</td>
                    <td>{$row['info_jour']}</td>
		    <td>{$row['info_capteur']}</td>
                    </tr>\n";
                }
...
<hr>
    <section id="d1">
		...
		if ($jour != "") {
 		...
 		$reqsql = $reqsql." ORDER BY info_jour DESC, info_heure DESC;";
                $query = mysqli_query($mysqli, $reqsql)
                or die (mysqli_error($mysqli));
		echo "<p style='text-align:center'> Historique de la journée du {$jour}</p>";
		echo "<div class='cards'>";

                while ($row = mysqli_fetch_array($query)) {
		    $meteo = $row['info_meteo'];
                    echo "<article class='card'>
              	    <header>
                        <h2>{$row['info_lieu']}<br>{$row['info_jour']}</h2>
              	    </header>
		    <img src='./Assets/{$row['info_meteo']}.png' alt='{$meteo}'/>
                    <div class='content'>
                        <p>{$row['info_meteo']}</p>
                    </div>
                    <table border='0'>
                        <td><td>Température : {$row['info_temperature']}°C</td><td>Pression : {$row['info_pression']} Pa</td></tr>
		...
		echo "<p style='display:none' id='indice'>0</p>";
		echo "</div>";
    ...
    <section id="d2" style="display:none">
        <form action="ajouter.php" method="post">
			...
			<option value="Pluie">Pluie</option>
              ...
              <li>
		<label for="hour">Heure de la prise &nbsp;:</label>
                <input type="time" name="iheure" required />
              </li>
              <li>
                <label for="jour" required>Date &nbsp;:</label>
