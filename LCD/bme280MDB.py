...
def main():
  ...
  #myHour = datetime.datetime.now().strftime('%H') + datetime.datetime.now().strftime('%M')
  ...
  addData("X",format(temperature,'.2f'),format(pressure,'.2f'),myDate,myHour,format(humidity,'.2f'),'Pau')
  ...
  while i<2:
  	...
  	lcd.putstr("Pression :")
  	lcd.move_to(0,1)
  	lcd.putstr(str(pressure)[0:6]+" hPa")
  	time.sleep(3)
  	lcd.clear()
  	lcd.move_to(0,0)
  	lcd.putstr("Humidite :")
  	lcd.move_to(0,1)
  	lcd.putstr(str(humidity)[0:5]+"%")
  	time.sleep(3)
  	lcd.clear()
  	i+=1
